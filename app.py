...
# ── AI: Gemini (when keyed) → Ollama local fallback ───────────────────────────
...
def ai_status():
    ...
    return jsonify({'ollama':ol,'ollama_model':OLLAMA_MODEL if ol else None,
                    ...
                    'active':'gemini' if GEMINI_API_KEY else ('ollama' if ol else 'none'),
                    'fallback':'ollama' if ol else None})
...
if __name__ == '__main__':
    ...
    print(f"   Ollama : {'✅ local fallback '+OLLAMA_MODEL if _ollama_available() else '⚠️  local fallback not running (ollama serve)'}")
