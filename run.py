...
def check_env():
    ...
    if not Path('.env').exists():
        if Path('.env.example').exists():
            ...
            print("   → Add GEMINI_API_KEY or use local Ollama fallback")
        else:
...
def check_ollama():
    try:
        ...
        print("✅ Ollama running (local AI fallback)")
    except:
        print("⚠️  Ollama not running — local fallback unavailable")
        print("   To enable local fallback: ollama serve  (in another terminal)")
